...
class sphere(shape):
    ...
    def intersect(self, ray, shadow):
        ...
        if l_dot_p < self.radius_sq:
            if shadow:
                return Intersection(ray.start_point, vector.negative(vector.normalized(l)), ray.direction,
                                    self.material)
            else:
                return

        thc = math.sqrt(self.radius_sq - d_square)
        inter_point = vector.add(ray.start_point, vector.multiply(ray.direction, tca - thc))
